...
{	
	const char *maxAttrNames[] = { "maxstr",
		"maxmagic",
		"maxdex",
		"maxvit"
	};

	return playerTable->GetInt(maxAttrNames[(int)attribute], (int)_pClass);
}
...
{
	const char *playerClassName = playerTable->GetValue("name", (int)_pClass);
	const char *sfxName = heroSpeechTable->GetValue(playerClassName, (int)speechId);

	_sfx_id soundEffect = SfxIdFromString(sfxName);
...
void Player::Say(HeroSpeech speechId, int delay) const
{
	...
	sfxdnum = SfxIdFromString(sfxName);
}
